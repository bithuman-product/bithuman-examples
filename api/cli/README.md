...
Run `bithuman <command> --help` for the flags. `<avatar>` is an agent code
(`A24EKJ8433`), a showcase name (`wise-pup`), or a file (`wise-pup.imx`).
...
| `bithuman doctor` | Check this machine. `--json` exits 0 when `.ready` is true. |
| `bithuman mcp` | MCP server over stdio (JSON-RPC), for AI agents. |
...
| [mac-app.sh](mac-app.sh) | `./mac-app.sh install` (runs `brew install`) then `./mac-app.sh run <model.imx>`. |
...
| `BITHUMAN_INSTRUCTIONS` | No | The voice agent's system prompt |
