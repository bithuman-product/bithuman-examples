...
<Info>
  **Requires an M3 or later (M5+ recommended) Mac.** The animator needs M3-class GPU memory bandwidth to sustain 25 FPS; M5+ is recommended for the smoothest experience. M1/M2 raises `ExpressionModelNotSupported` with install guidance.
</Info>
...
|---|---|
| macOS | 14 or later |
| Chip | Apple Silicon, **M3 or later (M5+ recommended)** (M1/M2 unsupported) |
| RAM | 16 GB |
...
<Steps>
  ...
  <Step title="Pick a face">
    Pass any portrait as `--identity` — local file or URL. The SDK encodes it on load (~300 ms on M3+):
...
| Symptom | Cause + fix |
|---|---|
| `ExpressionModelNotSupported` on an M1/M2 Mac | Animator requires M3+ memory bandwidth. Use an M3 or later (M5+ recommended), or the [self-hosted GPU deployment](/deployment/self-hosted-gpu) on Linux + NVIDIA. |
